  ...
  return (
    <div ref={rootRef} className="min-w-0" data-testid="daily-plan-scene-locations">
      <button
        ...
        className="flex min-h-9 w-full min-w-0 items-center rounded-[3px] border border-field-border bg-white px-2.5 text-left text-xs font-bold text-field-primary hover:border-field-primary hover:bg-field-light focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-field-primary"
        ...
        >
          <div className="min-h-0 flex-1 overflow-y-auto p-2">
            {pickerOptions.length > 0 ? (
                ...
                {pickerOptions.map((option) => {
                  ...
                  return (
                    <button
                      ...
                      className={`grid min-h-10 grid-cols-[minmax(0,1fr)_auto] items-center gap-2 rounded-[2px] border px-3 text-left text-xs font-bold disabled:cursor-not-allowed disabled:opacity-55 ${
                        isSelected
              ...
              <p className="px-2 py-5 text-center text-xs font-bold text-field-muted">
                선택할 대장소가 없습니다.
              </p>
            )}
          </div>
