    ...
    <div className="grid grid-cols-4 gap-1 rounded-full border border-field-border bg-white p-1">
      {filters.map((filter) => (
        <button
          ...
          className={cn(
            "min-h-[38px] rounded-full px-1 text-xs font-black leading-[1.25] tracking-[-0.05em] transition-[background-color,transform] active:scale-[0.98] focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-[#d7b95f] md:text-sm",
            value === filter ? "bg-field-primary text-white" : "bg-transparent text-field-muted hover:bg-field-soft"
          )}
        >
          <span className="font-condensed">{labels[filter]}</span>
        </button>
      ))}
    </div>
  );
