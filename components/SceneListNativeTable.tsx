...
const SceneNativeRow = memo(function SceneNativeRow({
  ...
  const sceneReorderGuideAnchorRef = useContextualGuideAnchor<HTMLTableRowElement>(
  ...
  const combinedRowRef = useCallback((element: HTMLTableRowElement | null) => {
    trProps.ref(element);
    sceneReorderGuideAnchorRef(element);
  }, [sceneReorderGuideAnchorRef, trProps.ref]);
  ...
  const mergeCell = (
    ...
    return (
      <td
        ...
        onPointerDown={(event) => {
          // Cell-range selection owns this pointer lifecycle. It must never
          // race the row's delayed reorder hold.
          event.stopPropagation();
          onBeginSelection(event, cell);
        }}
                  ...
                  return (
                    ...
                    >
                      <button
                        ...
                        onPointerDown={(event) => {
                          // Actor long-press/context editing owns this pointer;
                          // never let the row reorder timer run in parallel.
                          event.stopPropagation();
